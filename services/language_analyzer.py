...
class LanguageAnalyzer:
    ...
    def analyze_language_strength(self, document_text: str) -> Dict[str, any]:
        ...
        # Detect language for metadata and reporting
        ...
        avg_strength = (
            sum(s["strength_score"] for s in sentence_scores) / len(sentence_scores)
            if sentence_scores
            else 0
        )
        ...
        return {
            ...
            "ambiguous_sentences_count": len(ambiguous_sentences),
    ...
    def detect_language(self, text: str) -> str:
        ...
        Uses a lightweight heuristic based on common English and French terms.
        This avoids external dependencies and works sufficiently well for
        determining whether a document is primarily French or English.
        """
        if not text:
            return "unknown"
        ...
        # Common words for English vs French detection
        french_markers = [
            " le ",
            " la ",
            " les ",
        ...
        english_score = sum(1 for m in english_markers if m in lower)
    ...
    def _language_recommendations(self, language_type: str, avg_strength: float) -> List[str]:
        ...
        if language_type == "strong_mandatory":
            ...
            recommendations.append(
                "Language is balanced. Consider clarifying which items are mandatory vs. recommended."
            )
            if avg_strength < 0.6:
            ...
            recommendations.append(
                "Replace 'should' and 'may' with 'must' or 'shall' for enforceable requirements."
            )
        elif language_type == "ambiguous":
            recommendations.append("Language is unclear and ambiguous. Rewrite for clarity.")
            recommendations.append(
                "Use consistent terminology and mandatory language for all requirements."
            )
